...
public class Configure extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(extras != null) {
			...
			Log.d("configurationTag", "Extras == NULL");
		
		Log.d("configurationTag", "WidgetID: " + widgetID);
		final AppWidgetManager widgetManager = AppWidgetManager.getInstance(context);
		final RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.main);
		prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
		int userDate = prefs.getInt("inputDate" + widgetID, 01);
		int userMonth = prefs.getInt("inputMonth" + widgetID, 01);
		int userYear = prefs.getInt("inputYear" + widgetID, 2000);
		...
		//Set input restrictions on EditText fields
		et_date.setFilters(new InputFilter[] {new InputFilterMinMax("1", "31")});
...
//					custDateEdit.putInt("inputDate", Integer.valueOf(date.format(new Date())));
					...
					custDateEdit.putInt("inputDate" + widgetID, Integer.valueOf(et_date.getEditableText().toString()));
				}
...
//					custDateEdit.putInt("inputMonth", Integer.valueOf(month.format(new Date())));
					...
					custDateEdit.putInt("inputMonth" + widgetID, Integer.valueOf(et_month.getEditableText().toString()));
				}
...
//					custDateEdit.putInt("inputYear", Integer.valueOf(year.format(new Date())));
					...
					custDateEdit.putInt("inputYear" + widgetID, Integer.valueOf(et_year.getEditableText().toString()));
				}				
				custDateEdit.putInt("displaySettings" + widgetID, Integer.valueOf(displayFormat));
		...
		b_current.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				// GET text from edit text fields and commit to SharedPreferences
				SharedPreferences.Editor custDateEdit = prefs.edit();
				custDateEdit.putInt("inputDate" + widgetID, Integer.valueOf(date.format(new Date())));
				custDateEdit.putInt("inputMonth" + widgetID, Integer.valueOf(month.format(new Date())));
				custDateEdit.putInt("inputYear" + widgetID, Integer.valueOf(year.format(new Date())));
				custDateEdit.putInt("displaySettings" + widgetID, Integer.valueOf(displayFormat));
				Log.d("configurationTag", "Display format is " + Integer.valueOf(displayFormat));
				custDateEdit.commit();
				//Update widget
